...
class CurrentCall(val context: Context) {
    ...
    fun clientCall (city: String): Call<CurrentDataWeather> { // из clientCall передается параметр "название города" с типом данных стринг, Call - Синхронно отправить запрос и вернуть его ответ
        val network = Retrofit.Builder()
            .baseUrl("https://api.openweathermap.org/data/2.5/") //Базовая часть адреса
            .addConverterFactory(GsonConverterFactory.create()) //Конвертер, необходимый для преобразования JSON'а в объекты
            .build()
        val weatherServices = network.create(CurrentApiInterface::class.java) //Создаем объект, при помощи которого будем выполнять запросы
...
class ForecastCall(val context: Context) {
    private val apiKey = context.resources.getString(R.string.api_key) // Api ключ с сайта openweathermap
    fun clientCall (city: String): Call<ForecastDataWeather> { // из clientCall передается параметр "название города" с типом данных стринг, Call - Синхронно отправить запрос и вернуть его ответ
        val network = Retrofit.Builder()
            .baseUrl("https://api.openweathermap.org/data/2.5/") //Базовая часть адреса
            .addConverterFactory(GsonConverterFactory.create()) //Конвертер, необходимый для преобразования JSON'а в объекты
            .build()
        val weatherServices = network.create(ForecastApiInterface::class.java) //Создаем объект, при помощи которого будем выполнять запросы
        val weather = weatherServices.weatherCall(city, "metric", "ru", apiKey) //Создаем объект, из которого берем параметры в запрос и выполняем запрос с параментрами
        return weather //возвращаем результат запроса в переменную веазер
    }
}
