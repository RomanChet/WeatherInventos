...
// От меня: 
// Спасибо за прекрасное задание! В процессе его выполнения разобрался как работатает API вообще, с такими базовыми вещами как ReciclerView, Data Class и со всем причастным.
// У меня было время не отвлекаясь, понять эти моменты. Самое крутое то, что с каждым днем, решая множество мелких задач, всё быстрее понимал как их можно сделать.
// Всегда хотел реализовать себя в будущем как разработчик и вот, первый наношаг. Хочется набираться оыта, решать задачи, искать и находить решения каждый день!
